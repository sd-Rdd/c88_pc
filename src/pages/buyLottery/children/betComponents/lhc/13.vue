  ...
  <div class="lhc-bet-area">
    ...
    <div>
      <div class="bet-grid" v-for="(item,index1) in sublotteryCode.sixOdds" :key="index1" v-show="currentIndex==index1+1">
        <div v-if="item.code=='1301'">
          <div class="row title main-title">{{item.name}}</div>
          <div class="row">
            <div class="cell" style="width: 33.33%;" v-for="(item,index2) in item.children" :key="index2">
        ...
        <div v-else-if="item.code=='1302'||item.code=='1303'">
          <div class="row title">
            <div class="cell" style="width: 100%;">
              <div style="width: 20%;">半波</div>
              <div style="width: 20%;">赔率</div>
              <div style="width: 25%;">金额</div>
              <div style="width: 35%;">号码</div>
            </div>
          </div>
          <div class="row" v-for="(item,index2) in item.children" :key="index2">
            <div class="cell" style="width: 100%;">
              ...
              <div class="balls" style="width: 35%;box-sizing:border-box;">
                <span style="background-color: rgb(244, 89, 89); font-size: 12px; background-size: 100%; cursor: default; width: 28px; height: 28px; text-align: center; line-height: 28px; color: rgb(255, 255, 255); border: none; border-radius: 50%;"
                  v-for="(item,index) in item.numbers" :key="index">{{item}}</span>
              </div>
            </div>
          </div>
        </div>
        <div v-else-if="item.code=='1304'">
          <div class="row title">
            <div class="cell" style="width: 25%;">
              <div style="width: 25%;">号码</div>
              <div style="width: 25%;">赔率</div>
          ...
          <div class="cell" style="width: 25%;" v-for="(item,index2) in item.children" :key="index2">
            <div class="label" style="width: 25%;">{{item.name}}</div>
            <div class="odds false" style="width: 25%;" :class="{'item-selected':Inputprice[index1+''+Number(index2)]}">
              <span>{{item.odds}}</span>
            </div>
